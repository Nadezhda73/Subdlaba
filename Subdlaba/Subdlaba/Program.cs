...
namespace Subdlaba
{
    public class Program
    {
        ...
        {
            MainLogic logic = new MainLogic(new CustomerService(), new DeveloperService(), new DeveloperTrackerService(), new ProjectService(), new TimingService(), new TrackerService());
            Insert(logic);
            TrackerService newsLogic = new TrackerService();
            Stopwatch clock = new Stopwatch();
            clock.Start();
         //   logic.CreateTracker("Open", "Vot tak", 1);
         //   logic.ReadTracker();
